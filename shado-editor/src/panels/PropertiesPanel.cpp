    ...
    */
	...
	void PropertiesPanel::drawComponents(Entity entity) {
		...
		drawComponent<PrefabInstanceComponent>("Prefab Source", entity, [](PrefabInstanceComponent& prefabInstanceComponent) {
			...
			UI::Text("Prefab Element ID: %llu", prefabInstanceComponent.prefabEntityUniqueId);
		});
	...
	void ScriptPrefabRenderer::onRenderSceneStopped(const ScriptTypeRendererDataStopped& context) {
		...
		if (scriptModifiedFields.find(fieldName) != scriptModifiedFields.end()) {
			...
			if (data.id != 0) {
				...
				ScopedStyleColor textColor(ImGuiCol_Text, ImVec4(0.9f, 0.2f, 0.3f, 1.0f), prefab == nullptr && prefab->root);
				std::string prefabTextValue = prefab ? prefab->root.getComponent<TagComponent>().tag : std::to_string(data.id);
				prefabTextValue += " (prefab)";
				
				UI::InputTextControl(fieldName + "##prefabid", prefabTextValue, ImGuiInputTextFlags_ReadOnly);
			} else {
				std::string prefabIdStr = "(empty)";
				UI::InputTextControl(fieldName + "##prefabid", prefabIdStr, ImGuiInputTextFlags_ReadOnly);
			}
