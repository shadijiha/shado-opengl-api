    ...
    public:
        ...
        uint64_t GetLength() const {
            if (m_Type->IsSZArray()) {
                ...
                SHADO_CORE_ASSERT(false, "Invalid field type for `GetLength()`");
                return 0;
            }
        ...
        std::string GetValue() const {
            if (m_Instance) {
                ...
                std::string retval;
                retval.resize(m_ValueBuffer.Size - 1);
                memcpy(retval.data(), m_ValueBuffer.Data, m_ValueBuffer.Size - 1);
                return retval;
            }
        }
        ...
        std::string GetValue(uint32_t index) const {
            ...
            SHADO_CORE_ASSERT(false, "String array fields not currently supported");
            return {};
        }
        ...
        void SetValue(const std::string& value) {
            if (m_Instance) {
                m_Instance->SetFieldValue(m_Name, Coral::String::New(value));
            }
            else {
        ...
        void SetValue(const T& value, uint64_t index) {
            SHADO_CORE_ASSERT(m_Type->IsSZArray(), "");
        ...
        template <>
        void SetValue(const std::string& value, uint64_t index) {
            SHADO_CORE_ASSERT(m_Type->IsSZArray(), "");
