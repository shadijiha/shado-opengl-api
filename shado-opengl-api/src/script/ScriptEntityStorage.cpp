...
namespace Shado {
    ...
    void ScriptStorage::ShutdownEntityStorage(UUID scriptID, UUID entityID) {
        ...
        SHADO_CORE_ASSERT(scriptEngine.IsValidScript(scriptID), "");
        SHADO_CORE_ASSERT(EntityStorage.contains(entityID), "");
    ...
    void ScriptStorage::CopyTo(ScriptStorage& other) const {
        ...
        for (const auto& [entityID, entityStorage] : EntityStorage) {
            if (!scriptEngine.IsValidScript(entityStorage.ScriptID)) {
                SHADO_CORE_ERROR("Cannot copy script data for script ID {}. The script is no longer valid",
    ...
    void ScriptStorage::CopyEntityStorage(UUID entityID, UUID targetEntityID, ScriptStorage& targetStorage) const {
        ...
        if (dstStorage.ScriptID != srcStorage.ScriptID) {
            SHADO_CORE_ERROR(
                "Cannot copy script storage from entity {} to entity {} because they have different scritps!",
    ...
    void ScriptStorage::InitializeFieldStorage(EntityScriptStorage& storage, uint32_t fieldID,
