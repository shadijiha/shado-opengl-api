	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					SHADO_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
			...
			} else
			{
				if (Log::getCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
				{
					SHADO_CORE_ERROR("Instrumentor could not open results file '{0}'.", filepath);
				}
			}
		}
